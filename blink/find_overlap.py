...
def load_test_data(path):
    ...
    with open(path, 'rt') as f:
        ...
        for line in f:
            ...
            title = sample['title']
            mark_down_text = sample.get("text", "").strip()
            html = markdown(mark_down_text)
...
def run(args, test_data, logger):
    ...
    # bi-encoder step
    (labels, nns, bi_encoder_score, candidate_encoding,
     title2id, id2title, id2text, faiss_indexer) = bi_encoder_step(args, samples, keep_all, logger)

    print("cross encoder step...")
    l, predictions, cross_encoder_scores = cross_encoder_step(args, samples, labels, nns, id2title,
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "entity_catalogue": models_path + "cs_related_wiki_and_pwc_entities.jsonl",  #"entity.jsonl",
    ...
    #data_to_link = load_test_data("myKB-temp.jsonl")

    #title2id, id2title, id2text, predictions, cross_encoder_scores = run(args, data_to_link, logger)

    #save_var("find_overlap_results-temp.pckl", [title2id, id2title, id2text, predictions, cross_encoder_scores])
